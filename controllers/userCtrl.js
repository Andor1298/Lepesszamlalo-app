...
async function Registration(){
    ...
    if(passfield.value != confirmpassField.value){
        ShowAlert("A megadott jelszavak nem egyeznek meg!", "alert-danger")
        return
    }
    ...
    try{
        ...
        if (res.status == 200){
            ...
            ShowAlert("Sikeresa  regisztráció!", "alert-success")
        }
    }
    catch(err){
...
async function Login(){
    ...
    try{
        ...
        ShowAlert("Sikeres a belépés!","alert-success")
        user = await res.json()
...
async function ChangePass(){
    ...
    if (oldpassfield.value == NewPassField.value){
        ShowAlert("Az új jelszó nem egyezhet meg a régivel!", "alert-danger")
        return
    }
    try {
        const loggedUser = JSON.parse(sessionStorage.getItem('loggedUser'))
